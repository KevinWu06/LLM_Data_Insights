...
import useBannerImageMapServier from './excel_banner_images_servier';
import useBannerImageMapAUV from './excel_banner_images_auv';
...
const ServierBannerVisuals = () => {
  const { bannerImageMap, needsConsent, requestConsent } = useBannerImageMapServier();
  const banners = Object.entries(bannerImageMap)
    .filter(([title, url]) => url && url.trim() !== '');
  const isLoading = !needsConsent && Object.keys(bannerImageMap).length === 0;
...
export default { ServierBannerVisuals, AUVBannerVisuals }; 